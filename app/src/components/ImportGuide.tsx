...
const FILE_KINDS: FileKind[] = [
  ...
  },
  {
    ...
    need: 'either',
    recommended: true,
    what: '特定口座の損益が1ファイルに入っています（現物・信用・米国株・投信）',
    why: '商品指定を切り替える必要がなく、1回で済みます。特定口座の分はこれ1本で揃うので、下の米国株・投資信託はNISA口座で売却した場合だけ必要です。ファイル名は約定履歴と同じ SaveFile_ で始まりますが、中身が違うので取り込めば自動で判別されます',
  ...
  },
  {
    ...
    label: '米国株式',
    need: 'optional',
    what: 'NISA口座で売った米国株の損益',
    why: '特定口座の分は上の特定口座損益明細に入っているので不要です。NISA口座で米国株を売却したときだけ落としてください。一度取り込めば残ります',
  ...
  },
  {
    ...
    label: '投資信託',
    need: 'optional',
    what: 'NISA口座で解約した投資信託の損益',
    why: '特定口座の分は上の特定口座損益明細に入っています。NISA（つみたて投資枠・成長投資枠）で解約した分はここにしか無いので、該当があれば落としてください。一度取り込めば残ります',
